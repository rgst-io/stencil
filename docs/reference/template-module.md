...
This directory is used for storing all of the go-templates that a module owns. By default a file that doesn't have a `.tpl` or `.tplb` extension will be ignored by stencil.
...
- Files with a `.tplb` extension are treated as raw binary files and are written verbatim (not treated as templates) to the base of the execution directory of stencil, minus the `templates` directory and `.tplb` extension.
